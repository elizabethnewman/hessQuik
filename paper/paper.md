...
authors:
  - name: Lars Ruthotto^[co-first author] # note this makes a footnote saying 'co-first author'
    orcid: 0000-0003-0803-3299
    affiliation: "1" # (Multiple affiliations must be quoted)
  - name: Elizabeth Newman^[co-first author] # note this makes a footnote saying 'co-first author'
    affiliation: 1
...
# Summary
`hessQuik` provides efficient methods to compute second-order derivatives (Hessians) of functions formed via composition, such as neural networks, without automatic differentiation (AD). 
Hessians are challenging to compute efficiently with AD and cumbersome to derive and debug analytically.  Hence, many algorithms approximate Hessian information, resulting in suboptimal performance.  To address these challenges of second-order derivative computation, `hessQuik` provides efficient, analytic Hessians with GPU acceleration for a variety of network layers that can be used seamlessly with PyTorch [@pytorch].
% removing the need for inaccurate approximations of second-order information and expensive passes through the computational graph.  
The package is user-friendly and flexible by design -- users can construct many-layered networks with a variety of layer types, including fully-connected layers, residual layers, and input convex layers, and use various nonlinear activation functions, including hyperbolic tangent, sigmoid, and quadratic.  
...
Despite being a powerful tool to compute gradients, AD encounters computational obstacles when computing second-order derivatives.
Knowledge of second-order derivatives is paramount in many growing fields, such as physics-informed neural networks (PINNs) [@Raissi:2019hv], mean-field games [@Ruthotto9183], generative modeling [@ruthotto2021introduction], and adversarial learning [@papernot2016limitations].  In addition, second-order derivative information can provide insight into the optimization problem solved to form a good DNN model [@olearyroseberry2020illposedness].  However, Hessians are challenging to compute efficiently with automatic differentiation and cumbersome to derive and debug analytically.  Hence, many algorithms approximate Hessian information, resulting in suboptimal performance.  To address these challenges of second-order derivative computation, `hessQuik` provides efficient, analytic Hessians with GPU acceleration for a variety of network layers that can be used seamlessly with PyTorch.

# The Chain Rule for Composed Functions

We consider complex functions constructed via composition of simpler functions, which we call \emph{layers}.  
Specifically, let $f:\Rbb^{n_0} \to \Rbb^{n_\ell}$ be a twice continuously-differentiable function defined as
	\begin{align}
	f = g_{\ell} \circ g_{\ell - 1} \circ \cdots \circ g_1, \quad\text{where} \quad g_i: \Rbb^{n_{i-1}} \to \Rbb^{n_i} \quad \text{for }i=1,\dots, \ell.
	\end{align}
Here, $g_i$ represents the $i$-th layer and $n_i$ is the number of hidden features on the $i$-th layer.   We call $n_0$ the number of input features and $n_{\ell}$ the number of output features.  `hessQuik` provides many common layer types, including fully-connected and residual layers.  
We note that each layer is parameterized by weights which we can tune by solving an optimization problem. Because `hessQuik` computes derivatives for the network inputs for a given set of weights, we omit the network weights from our notation. 
...
Computing derivatives in forward mode means we sequentially build the gradient and Hessian \emph{during forward propagation}; that is, when we form $\bfu_i$, we simultaneously form the corresponding gradient and Hessian information.  
We start by computing the gradient and Hessian of the first layer with respect to the inputs; that is, 
    \begin{align}
...
The computational efficiency of computing derivatives is proportional to the number of input features $n_0$ and the number of output features $n_{\ell}$.  
The heuristic we use is if $n_0 < n_\ell$, we compute derivatives in forward mode, otherwise we compute derivatives in backward mode. 
Our implementation automatically selects the mode of derivative computation based on this heuristic.
Users have the option to select their preferred mode of derivative computation if desired. 
...
We compare the time to compute the Hessian of a neural network with respect to the input features of three approaches: 
    \begin{itemize}
        \item `hessQuik` (our implementation): AD-free method
        \item `PytorchAD`: PyTorch's automatic differentiation method following the implementation in [@huang2021convex]
        \item `PytorchHessian`: PyTorch's built-in [Hessian function](https://pytorch.org/docs/stable/generated/torch.autograd.functional.hessian.html)
    \end{itemize}
...
![Average time over $10$ trials to evaluate and compute the Hessian with respect to the input features for o\
ne output feature ($n_{\ell}=1$). Solid lines represent timings on the CPU and dashed lines are timings on the GPU. The circle markers are the timings obtained using `hessQuik` and produce the fastest timings for a various numbers \
of input features $n_0$. \label{fig:scalar}](img/hessQuik_timing_scalar.png){ width=80% }
