from hessQuik.networks import NN, NNLapQuik
from hessQuik.layers import singleLayer, singleLayerLapQuik
...
class fullyConnectedNNLapQuik(NNLapQuik):
    r"""
    Fully-connected network where every layer is a single layer.  Let :math:`u_0 = x` be the input into the network.
    The construction is of the form
    ...
    def __init__(self, widths: Union[Tuple, List], act: act.hessQuikActivationFunction = act.softplusActivation(),
        ...
        args = ()
        for i in range(len(widths) - 1):
            args += (singleLayerLapQuik(widths[i], widths[i + 1], act=deepcopy(act), **factory_kwargs),)

        super(fullyConnectedNNLapQuik, self).__init__(*args)
...
if __name__ == '__main__':
    ...
    fLapQuik = fullyConnectedNNLapQuik([d, 2, 5, 1], act=act.softplusActivation())
    ...
    print('======= LAPLACIAN =======')
    input_derivative_check_finite_difference_laplacian(fLapQuik, x, do_Laplacian=True, verbose=True)
