...
def device_and_gradient_mapping(f, network_wrapper='hessQuik', device='cpu'):
...
def create_network(in_features, out_features, width, depth, network_type='resnet',
    ...
    f, x_requires_grad = device_and_gradient_mapping(f, network_wrapper=network_wrapper, device=device)
    ...
    for idx_out, out_features in enumerate(out_feature_range):
        for idx_in, in_features in enumerate(in_feature_range):
            for idx_nex, nex in enumerate(nex_range):
                ...
                # setup network
                f, x_requires_grad = create_network(in_features, out_features, width, depth,  network_type=network_type,
